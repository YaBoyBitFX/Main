...
function Library:Create(options)
	...
	do
		...
		Gui["2"]["Size"] = UDim2.new(0, 250, 0, 150)
	...
	if (key.UserInputType == Enum.UserInputType.MouseButton1 or key.UserInputType == Enum.UserInputType.Touch)
		and Slider.Hover
	...
	if Library.MainFrameHover then
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch)
			and not Library.Sliding
		then
			local ObjectPosition = Vector2.new(
				input.Position.X - Gui["2"].AbsolutePosition.X,
				input.Position.Y - Gui["2"].AbsolutePosition.Y
			)

			local activeInput = input -- store the input that started the drag

			local connection
			connection = RunService.RenderStepped:Connect(function()
				-- Stop dragging if input has ended
				if not UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
					and activeInput.UserInputState == Enum.UserInputState.End
				then
				...
				if not Library.Sliding then
					local currentPos = (activeInput.UserInputType == Enum.UserInputType.Touch and activeInput.Position) or Vector2.new(Mouse.X, Mouse.Y)

					Library:Tween(Gui["2"], {
						Goal = {
							Position = UDim2.fromOffset(
								currentPos.X - ObjectPosition.X + (Gui["2"].Size.X.Offset * Gui["2"].AnchorPoint.X),
								currentPos.Y - ObjectPosition.Y + (Gui["2"].Size.Y.Offset * Gui["2"].AnchorPoint.Y)
							),
						},
			...
			-- End the drag when touch or mouse input ends
